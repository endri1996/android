...
public class Transaksihelper extends SQLiteOpenHelper {
    ...
    public void insertTransaksi(String nama,int jenis, int jumlah , String keterangan){
        ...
        values.put(Transaksi.COL_NAME,nama);
    ...
    public List<Transaksi> getTransaksi(){
        ...
        String [] projection = {Transaksi._ID, Transaksi.COL_NAME, Transaksi.COL_JENIS, Transaksi.COL_JUMLAH, Transaksi.COL_KETERANGAN};
        String sortOrder = Transaksi._ID;
        Cursor cursor = db.query(Transaksi.TABLE_NAME, projection, null,null,null,null,sortOrder);

        Transaksi newTrans;
        while (cursor.moveToNext()){
            newTrans = new Transaksi(cursor.getString(cursor.getColumnIndex(Transaksi.COL_NAME)),
